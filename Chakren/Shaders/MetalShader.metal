...
vertex VertexOut chakra_vertex(VertexIn in [[stage_in]],
                               constant ChakraUniforms &uniforms [[buffer(1)]])
{
    VertexOut out;
    float scale = 1.0 + 0.1 * sin(uniforms.time * uniforms.frequency);
    out.position = float4(in.position * scale, 0, 1);
